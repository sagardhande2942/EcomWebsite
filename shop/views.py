...
def countdown():
    ...
    # DateCounter(dateNow=datetime.now(), dateEnd= datetime.now() + timedelta(minutes=1))
    dateCounter = DateCounter.objects.filter(id=2)
    dateCounter.update(dateNow=datetime.now(),
                       dateEnd=datetime.now() + timedelta(minutes=1))
    print('Hii here')
    globalDailyDeals = dailyDeals(5)
...
def index(request, num):
    ...
    try:
        ...
        DateCounter(dateNow=datetime.now(),
                    dateEnd=datetime.now() + timedelta(minutes=1)).save()
        DateCounter(dateNow=datetime.now(),
                    dateEnd=datetime.now() + timedelta(minutes=1)).save()
    ...
    maxProduct.reverse()
    for i in maxProduct:
        maxProductInstance.append(
            Product.objects.filter(product_id=int(i[2:])))
    ...
    for cat in cats:
        ...
        nslides2 = n
        allprods.append([prod, range(1, nslides1), nslides1, range(
            1, nslides), nslides, range(1, nslides2), nslides2])
...
def showCart(request):
    ...
    allprods = Product.objects.values("product_id", "product_desc", "price",
                                      "image", "product_pubs_date", "product_name", 'discount', 'offers')
    username = request.user.username
    p = []
    cnt = 0
    for j in xProds:
        ...
        # print(i)
        for i in j:
            p.append([i.product_id, i, (i.price + i.price * (i.discount / 100)),
                     daysRequiredInCart, sellersList[random.randint(0, 4)]])
    # print(p)
    param = {
        'p': p,
...
def search(request):
    ...
    if request.method == "POST":
        ...
        try:
            url = "https://www.flipkart.com/search?q=" + aReal.lower() + \
            ...
            for i in dd:
                if a in i.category.lower():
                    for z in Product.objects.filter(category=i.category):
                        ...
                        if len(b) > 50:
                            break
                        b.append(Product.objects.filter(
            ...
            if not len(b):
                K = len(a) // 2
                test_str = fl
                res = [test_str[i: j] for i in range(len(test_str)) for j in range(
                    i + 1, len(test_str) + 1) if len(test_str[i:j]) == K]
                # print('hiii in ssearch')
                d = Product.objects.all()
                dd = []
                b = []
...
def tracker(request):
    ...
    n10 = 0
    for i in b11:
        avtime.append([i.purd, i.lang, i.lat, i.days,
                      i.state, i.lato, i.lango])
        n10 += 1
    # print(avtime)
    # print(n10, 'is n10')
    states = [
    ...
    for i in diffDict:
        ...
        for j in m:
            ...
            if not len(mb):
                continue
            z2.append([mb, j[1], avtime[n10 - i - 1][4], avtime[n10 - i - 1][1], avtime[n10 - i - 1][2],
                      avtime[n10 - i - 1][0], avtime[n10 - i - 1][3], avtime[n10 - i - 1][6], avtime[n10 - i - 1][5]])
            # assert n10 - i - 1 >= 0
        # n10-=1
        finallist.append(z2)
    # print(finallist)
...
def trackCart(request):
    if request.method == 'POST':
        ...
        a = Comments.objects.filter(
            usr_id=request.user.id, product_id=product_id)
        ...
        if not a:
            ...
            a.update(cmt_title=cmt_title, cmt_desc=cmt_desc,
                     rating=z[0].rating, edited=True, edit_time=datetime.now())
    # #print('hiids')
    username = request.user.username
    a11 = ExtendedUser.objects.filter(usr=request.user)
    b11 = PurchaseDate.objects.filter(purdate=a11[0])
    ...
    for i in diffDict:
        ...
        for j in m:
            ...
            # print(num)
            mb = Product.objects.filter(product_id=int(num))
            z4 = Comments.objects.filter(
                usr_id=request.user.id, product_id=mb[0])
            try:
                print('This is the comment ',
                      z4[0].cmt_title, ' ', z4[0].cmt_desc)
            except:
                pass
            zxcv = random.randint(0, 7)
            # print(n10 - i - 1)
...
def getAddress(request):
    ...
    if request.method == 'POST':
        ...
        PDFromgetAddtoSuccessPay = PurchaseDate(purdate=c[0], purd=d, state=states[f][0], lato=states[f][2], lango=states[f][1], lat=str(
            b['lat']), lang=str(b['lon']), days=daysRequiredInCart)
        # PDFromgetAddtoSuccessPay.save()
    return JsonResponse({'hii': 'bye'})
...
def sellWithUs(request):
    if request.method == 'POST':
        try:
            ...
            se1 = driver.find_element_by_xpath(
            ...
            for i in range(1, 2):
                try:
                    ...
                    if (prodtype == 1 and i <= 4):
                        driver.find_element_by_xpath(
                            '//*[@id="container"]/div/div[3]/div[1]/div[2]/div[2]/div/div['+str(i)+']/div/a[2]').click()
                    elif (prodtype == 1 and i > 4 and i <= 8):

                        driver.find_element_by_xpath(
                            '//*[@id="container"]/div/div[3]/div[1]/div[2]/div[3]/div/div['+str(a)+']/div/a[2]').click()
                        a += 1
                    elif (prodtype == 1 and i > 8 and i <= 12):

                        driver.find_element_by_xpath(
                            '//*[@id="container"]/div/div[3]/div[1]/div[2]/div[4]/div/div['+str(b)+']/div/a[2]').click()
                        b += 1
                    elif (prodtype == 1 and i > 12 and i <= 16):

                        driver.find_element_by_xpath(
                            '//*[@id="container"]/div/div[3]/div[1]/div[2]/div[5]/div/div['+str(c2)+']/div/a[2]').click()
                        c2 += 1
                    elif (prodtype == 1 and i > 16 and i <= 20):
                        driver.find_element_by_xpath(
                            '//*[@id="container"]/div/div[3]/div[1]/div[2]/div[6]/div/div['+str(d2)+']/div/a[2]').click()
                        d2 += 1
                    elif (prodtype == 1 and i > 20 and i <= 24):
                        driver.find_element_by_xpath(
                            '//*[@id="container"]/div/div[3]/div[1]/div[2]/div[7]/div/div['+str(d3)+']/div/a[2]').click()
                        d3 += 1
                except:
                    try:
                        prodtype = 2
                        if (prodtype == 2):
                            ...
                            print('hii')
                            print(i+1)
                            driver.find_element_by_xpath('//*[@id="container"]/div/div[3]/div[1]/div[2]/div['+str(
                                i+1)+']/div/div/div/a/div[2]/div[1]/div[1]').click()
                    except:
                        prodtype = 3
                        try:
                            ...
                            if (prodtype == 3 and i > 4 and i <= 8):
                                print(i)
                                driver.find_element_by_xpath(
                                    '//*[@id="container"]/div/div[3]/div[1]/div[2]/div[3]/div/div['+str(a)+']/div/div/a[1]').click()
                                a += 1
                            elif (prodtype == 3 and i > 8 and i <= 12):

                                driver.find_element_by_xpath(
                                    '//*[@id="container"]/div/div[3]/div[1]/div[2]/div[4]/div/div['+str(b)+']/div/div/a[1]').click()
                                b += 1
                            elif (prodtype == 3 and i > 12 and i <= 16):

                                driver.find_element_by_xpath(
                                    '//*[@id="container"]/div/div[3]/div[1]/div[2]/div[5]/div/div['+str(c2)+']/div/div/a[1]').click()
                                c2 += 1
                            elif (prodtype == 3 and i > 16 and i <= 20):
                                driver.find_element_by_xpath(
                                    '//*[@id="container"]/div/div[3]/div[1]/div[2]/div[6]/div/div['+str(d2)+']/div/div/a[1]').click()
                                d2 += 1
                            elif (prodtype == 3 and i > 20 and i <= 24):
                                driver.find_element_by_xpath(
                                    '//*[@id="container"]/div/div[3]/div[1]/div[2]/div[7]/div/div['+str(d3)+']/div/div/a[1]').click()
                                d3 += 1
                        except:
                ...
                if (prodtype == 3):
                    images = driver.find_elements_by_xpath(
                        '//*[@id="container"]/div/div[3]/div[1]/div[1]/div[1]/div/div[1]/div[2]/div[1]/div[2]/div/img')
                    for image in images:
                ...
                try:
                    kk = 0
                    diss1 = ''
                    diss = driver.find_element_by_xpath(
                        '//*[@id="container"]/div/div[3]/div[1]/div[2]/div[2]/div/div[3]/div[1]/div/div[3]/span').text
                    print("sd1")
                    for di in diss:
                        if (di == '%'):
                            break
                    ...
                    try:
                        diss1 = ''
                        diss = driver.find_element_by_xpath(
                            '//*[@id="container"]/div/div[3]/div[1]/div[2]/div[2]/div/div[4]/div[1]/div/div[3]/span').text
                        for di in diss:
                            if (di == '%'):
                                break
                            diss1 += di
                ...
                try:
                    ...
                    print("hii")
                    subcategory = driver.find_element_by_xpath(
                        '//*[@id="container"]/div/div[3]/div[1]/div[2]/div[2]/div[1]/div/div[3]/a').text
                    print('bye')
                ...
                try:
                    subcategory1 = driver.find_element_by_xpath(
                        '//*[@id="container"]/div/div[3]/div[1]/div[2]/div[2]/div[1]/div/div[4]/a').text
                except:
            ...
            for i in range(1):
                ...
                open(os.getcwd() + '/shop/images2/' + cat+'/'+cat +
                     ''+str(rand_token)+'.png', 'wb').write(r.content)
            ...
            return JsonResponse({'error': "Sorry there was a error processing your request, try different product" + str(error)})
...
def saveProduct(request):
    if request.method == 'POST':
        ...
        driver.find_element_by_xpath(
            '//*[@id="content-main"]/div[2]/table/tbody/tr[3]/td[1]/a').click()
        time.sleep(3)
        try:
            os.mkdir(os.getcwd() + '/shop/images2/' + category1+'')
        except:
        ...
        for i in range(1):
            ...
            driver.execute_script(
                'document.getElementById("id_discount").value = '+str(config['dis'])+'')
            # dis.send_keys(config["dis"])
            print('Added Discount Successfully')
            ...
            subcat1 = driver.find_element_by_xpath(
                '//*[@id="id_subcategory1"]')
            subcat1.send_keys(config["sub_cat1"])
            print('Added SubCat1 Successfully')
            ...
            print(pri)
            driver.execute_script(
                'document.getElementById("id_price").value = '+str(pri)+'')
            print('Added price Successfully')

            ratingRand = random.randint(1, 5)
            driver.execute_script(
                'document.getElementById("id_rating").value = '+str(ratingRand)+'')

            driver.find_element_by_id("id_image").send_keys(
                os.getcwd()+"/shop/images2/"+cat+"/"+config["cat"]+""+rand_token+".png")
            driver.find_element_by_id("id_image1").send_keys(
                os.getcwd()+"/shop/images2/"+cat+"/"+config["cat"]+""+rand_token+".png")
            print('Added Image Successfully')
            imageLink = config["cat"]+""+rand_token+".png"
            driver.find_element_by_xpath(
                '//*[@id="product_form"]/div/div/input[2]').click()
            # driver.find_element_by_xpath('//*[@id="product_form"]/div/div/input[1]').click()
